...
class MemoryLinkGame {
  ...
  initializeStatsToggle() {
    // Set correct icon based on screen size
    if (window.innerWidth <= 768) {
      // Mobile - start with chart icon
      this.statsToggle.innerHTML = '<i class="fas fa-chart-line"></i>';
    } else {
  ...
  bindEvents() {
    ...
    // Handle window resize for responsive stats toggle
  ...
  showSequence() {
    ...
    // Disable all button interactions during sequence display
  ...
  handleButtonClick(index) {
    ...
    if (!isCorrect) {
      ...
      // Correct input - use quick feedback animation and color-specific sound
      this.flashButtonQuick(button);
  ...
  flashButtonWithRipple(button) {
    ...
    button.classList.add('gentle-pulse', 'ripple', 'active', 'click-feedback');
    
    // Remove animation classes after animation completes
    setTimeout(() => {
      button.classList.remove('gentle-pulse', 'ripple', 'active', 'click-feedback');
    }, 800); // Increased duration for smoother animation
  }

  flashButtonWithShake(button) {
    // Add aggressive visual feedback for wrong clicks
    button.classList.add('shake-ripple', 'active', 'click-feedback');
    
    // Remove animation classes after animation completes
    setTimeout(() => {
      button.classList.remove('shake-ripple', 'active', 'click-feedback');
    }, 800); // Increased duration for smoother animation
  }

  flashButtonQuick(button) {
    // Quick feedback for rapid clicking during input phase
    button.classList.add('quick-flash', 'active');
    
    // Remove animation classes quickly to allow rapid clicking
    setTimeout(() => {
      button.classList.remove('quick-flash', 'active');
    }, 200); // Short duration for rapid feedback
  }
  ...
  toggleStatsPanel() {
    if (window.innerWidth <= 768) {
      ...
      const icon = this.statsToggle.querySelector('i');
